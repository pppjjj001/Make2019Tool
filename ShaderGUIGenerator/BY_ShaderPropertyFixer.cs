...
//------------------------------------------增加根据GUI Toggle 还原shader原始[Toggle]标签------------------------------------------
using System;
using System.Collections.Generic;
using System.IO;
using System.Linq;
...
namespace UnityEditor
{
    public class BY_ShaderPropertyFixer : EditorWindow
    {
        ...
        private class PropertyFixInfo
        {
            ...
            public string[] keywordEnumKeywords;
            public string[] enumValues;
            public bool isToggle;              // 是否为 Toggle 类型
            public bool isToggleFromGUI;       // 是否从 GUI 代码中检测到的 Toggle
            public bool isKeywordEnum;
            public bool needsAttributeFix;
            public bool needsDisplayNameFix;
            public string tooltip;
        ...
        private void OnGUI()
        {
            ...
            EditorGUILayout.HelpBox(
                "此工具可以：\n" +
                "1. 根据ShaderGUI代码中的Styles还原Shader属性的显示名称\n" +
                "2. 从GUI代码中识别Toggle属性并自动补全[Toggle(KEYWORD)]声明\n" +
                "3. 自动补全KeywordEnum属性缺少的声明\n\n" +
                "分析顺序：先识别Toggle → 再匹配关键字 → 最后补全属性", 
                MessageType.Info);
        ...
        private void DrawAnalysisResults()
        {
            ...
            int needsDisplayNameFix = propertyFixInfos.Values.Count(p => p.needsDisplayNameFix);
            int needsAttributeFix = propertyFixInfos.Values.Count(p => p.needsAttributeFix);
            int toggleCount = propertyFixInfos.Values.Count(p => p.isToggle || p.isToggleFromGUI);
            int keywordEnumCount = propertyFixInfos.Values.Count(p => p.isKeywordEnum);
            
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField($"Toggle属性: {toggleCount}", EditorStyles.miniLabel);
            EditorGUILayout.LabelField($"KeywordEnum: {keywordEnumCount}", EditorStyles.miniLabel);
            EditorGUILayout.EndHorizontal();
            ...
            // 表头
            EditorGUILayout.BeginHorizontal("box");
            EditorGUILayout.LabelField("属性名", EditorStyles.boldLabel, GUILayout.Width(140));
            EditorGUILayout.LabelField("类型", EditorStyles.boldLabel, GUILayout.Width(80));
            EditorGUILayout.LabelField("当前", EditorStyles.boldLabel, GUILayout.Width(140));
            EditorGUILayout.LabelField("建议", EditorStyles.boldLabel, GUILayout.Width(160));
            EditorGUILayout.LabelField("状态", EditorStyles.boldLabel, GUILayout.Width(80));
            EditorGUILayout.EndHorizontal();
            ...
            foreach (var kvp in propertyFixInfos)
            {
                ...
                else if (info.isToggleFromGUI || info.isKeywordEnum)
                {
                ...
                EditorGUILayout.LabelField(info.propertyName, GUILayout.Width(140));
                
                // 类型
                string typeStr = "-";
                if (info.isToggleFromGUI) typeStr = "Toggle(GUI)";
                else if (info.isToggle) typeStr = "Toggle";
                else if (info.isKeywordEnum) typeStr = "KeywordEnum";
                EditorGUILayout.LabelField(typeStr, EditorStyles.miniLabel, GUILayout.Width(80));
                ...
                EditorGUILayout.LabelField(currentInfo, EditorStyles.wordWrappedMiniLabel, GUILayout.Width(140));
                ...
                if (!string.IsNullOrEmpty(info.associatedKeyword) && !info.needsAttributeFix)
                {
                    if (!string.IsNullOrEmpty(suggestedInfo)) suggestedInfo += "\n";
                    suggestedInfo += $"关键字: {info.associatedKeyword}";
                }
                if (string.IsNullOrEmpty(suggestedInfo)) suggestedInfo = "-";
                EditorGUILayout.LabelField(suggestedInfo, EditorStyles.wordWrappedMiniLabel, GUILayout.Width(160));
        ...
        private void AnalyzeShader()
        {
            ...
            // Step 1: 解析Shader中的Properties
            Debug.Log("=== Step 1: 解析Shader Properties ===");
            ParseShaderProperties(originalShaderContent);
            
            // Step 2: 解析Shader中的关键字
            Debug.Log("=== Step 2: 解析Shader关键字 ===");
            ParseShaderKeywords(originalShaderContent);
            
            // Step 3: 如果有ShaderGUI脚本，优先解析Toggle，再解析关键字映射
            if (shaderGUIScript != null)
            {
                string guiScriptPath = AssetDatabase.GetAssetPath(shaderGUIScript);
                if (!string.IsNullOrEmpty(guiScriptPath) && File.Exists(guiScriptPath))
                {
                    ...
                    // Step 3.1: 优先识别Toggle属性
                    Debug.Log("=== Step 3.1: 从GUI代码识别Toggle属性 ===");
                    ParseTogglePropertiesFromGUI(guiContent);
                    
                    // Step 3.2: 解析Styles中的显示名称
                    Debug.Log("=== Step 3.2: 解析Styles显示名称 ===");
                    ParseStylesFromGUI(guiContent);
                    
                    // Step 3.3: 解析关键字映射
                    Debug.Log("=== Step 3.3: 解析关键字映射 ===");
                    ParseKeywordMappingsFromGUI(guiContent);
                    
                    // Step 3.4: 解析KeywordEnum
                    Debug.Log("=== Step 3.4: 解析KeywordEnum ===");
                    ParseKeywordEnumFromGUI(guiContent);
                }
            }
            
            // Step 4: 分析并确定需要修复的内容
            Debug.Log("=== Step 4: 分析修复需求 ===");
            AnalyzeFixRequirements();
            
            int toggleCount = propertyFixInfos.Values.Count(p => p.isToggle || p.isToggleFromGUI);
            int keywordEnumCount = propertyFixInfos.Values.Count(p => p.isKeywordEnum);
            
            Debug.Log($"分析完成: {propertyFixInfos.Count} 个属性, {toggleCount} 个Toggle, {keywordEnumCount} 个KeywordEnum, {detectedKeywords.Count} 个关键字");
        }
        ...
        /// 优先从GUI代码中识别Toggle属性
        /// 检测模式：
        /// 1. bool xxxEnabled = properties.xxx.floatValue >= 0.5f;
        /// 2. xxxEnabled = EditorGUILayout.Toggle(...)
        /// 3. properties.xxx.floatValue = xxxEnabled ? 1.0f : 0.0f;
        /// </summary>
        private void ParseTogglePropertiesFromGUI(string guiContent)
        {
            // 模式1: 检测 bool xxxEnabled = properties.xxx.floatValue >= 0.5f;
            var togglePattern1 = new Regex(
                @"bool\s+(\w+)Enabled\s*=\s*properties\.(\w+)\.floatValue\s*>=?\s*0\.5f",
                RegexOptions.Multiline);
            
            var matches1 = togglePattern1.Matches(guiContent);
            foreach (Match match in matches1)
            {
                string varName = match.Groups[2].Value;
                string propName = "_" + char.ToUpper(varName[0]) + varName.Substring(1);
                
                MarkAsToggleFromGUI(propName, varName);
            }
            
            // 模式2: 检测 EditorGUILayout.Toggle 配合 properties.xxx
            var togglePattern2 = new Regex(
                @"(\w+)Enabled\s*=\s*EditorGUILayout\.Toggle\s*\(",
                RegexOptions.Multiline);
            
            var matches2 = togglePattern2.Matches(guiContent);
            ...
            // 模式3: 检测 properties.xxx.floatValue = xxxEnabled ? 1.0f : 0.0f
            var togglePattern3 = new Regex(
                @"properties\.(\w+)\.floatValue\s*=\s*\w+Enabled\s*\?\s*1\.0f\s*:\s*0\.0f",
                RegexOptions.Multiline);
            
            var matches3 = togglePattern3.Matches(guiContent);
            ...
            // 模式4: 检测 EditorGUILayout.ToggleLeft
            var togglePattern4 = new Regex(
                @"(\w+)Enabled\s*=\s*EditorGUILayout\.ToggleLeft\s*\(",
                RegexOptions.Multiline);
            
            var matches4 = togglePattern4.Matches(guiContent);
            ...
            int toggleFromGUICount = propertyFixInfos.Values.Count(p => p.isToggleFromGUI);
            Debug.Log($"从GUI代码中识别到 {toggleFromGUICount} 个Toggle属性");
        }
        
        private void MarkAsToggleFromGUI(string propName, string varName)
        {
            // 尝试多种属性名格式
            string[] possibleNames = new string[]
            {
                propName,
                "_" + varName,
                "_" + char.ToUpper(varName[0]) + varName.Substring(1),
                "_" + varName.ToUpper(),
            };
            
            foreach (var name in possibleNames)
            {
                if (propertyFixInfos.ContainsKey(name))
                {
                    var info = propertyFixInfos[name];
                    if (!info.isToggle && !info.isToggleFromGUI)
                    {
                        info.isToggleFromGUI = true;
                        Debug.Log($"从GUI识别Toggle: {name}");
                    }
                    return;
                }
            }
        ...
        /// 解析Styles类中的显示名称
        /// </summary>
        private void ParseStylesFromGUI(string guiContent)
        {
            // 格式: public static GUIContent xxxText = new GUIContent("Display Name", "Tooltip");
            var stylePattern = new Regex(
                @"public\s+static\s+GUIContent\s+(\w+)Text\s*=\s*new\s+GUIContent\s*\(\s*""([^""]*)""\s*(?:,\s*""([^""]*)"")?\s*\)",
            ...
            foreach (Match match in matches)
            {
                ...
                foreach (var name in possibleNames)
                {
                    if (propertyFixInfos.ContainsKey(name))
                    {
                        ...
                        info.suggestedDisplayName = displayName;
                        info.tooltip = tooltip;
                        Debug.Log($"从Styles获取显示名称: {name} -> \"{displayName}\"");
                        break;
                    }
                }
            }
        ...
        /// 解析SetMaterialKeywords方法中的关键字映射
        /// </summary>
        private void ParseKeywordMappingsFromGUI(string guiContent)
        {
            // 格式: CoreUtils.SetKeyword(material, "KEYWORD_NAME", material.GetFloat("_PropertyName") == 1.0f);
            var keywordPattern = new Regex(
                @"CoreUtils\.SetKeyword\s*\(\s*material\s*,\s*""([^""]+)""\s*,[\s\S]*?material\.GetFloat\s*\(\s*""([^""]+)""\s*\)\s*[=!><]+\s*[\d\.]+f?\s*\)",
                RegexOptions.Multiline);
            ...
            foreach (Match match in keywordMatches)
            {
                ...
                if (propertyFixInfos.ContainsKey(propName))
                {
                    ...
                    // 如果是从GUI识别的Toggle，且还没有关联关键字
                    if ((info.isToggleFromGUI || info.isToggle) && string.IsNullOrEmpty(info.associatedKeyword))
                    {
                        info.associatedKeyword = keyword;
                        Debug.Log($"为Toggle属性 {propName} 关联关键字: {keyword}");
                    }
                    else if (string.IsNullOrEmpty(info.associatedKeyword))
                    {
                        info.associatedKeyword = keyword;
                    }
                }
            ...
            // 备选模式: SetKeyword(material, "KEYWORD", xxxEnabled);
            var keywordPattern2 = new Regex(
                @"(?:CoreUtils\.)?SetKeyword\s*\(\s*material\s*,\s*""([^""]+)""\s*,\s*(\w+)Enabled\s*\)",
                RegexOptions.Multiline);
            
            var keywordMatches2 = keywordPattern2.Matches(guiContent);
            
            foreach (Match match in keywordMatches2)
            {
                string keyword = match.Groups[1].Value;
                string varName = match.Groups[2].Value;
                ...
                string[] possibleNames = new string[] { propName, "_" + varName };
                ...
                foreach (var name in possibleNames)
                {
                    if (propertyFixInfos.ContainsKey(name))
                    {
                        ...
                        {
                            info.associatedKeyword = keyword;
                            Debug.Log($"为属性 {name} 关联关键字(模式2): {keyword}");
                        }
                        break;
                    }
                }
        ...
        /// 解析KeywordEnum的关键字列表
        /// </summary>
        private void ParseKeywordEnumFromGUI(string guiContent)
        {
            // 格式: int xxxValue = (int)material.GetFloat("_Xxx");
            var keywordEnumPattern = new Regex(
                @"int\s+(\w+)Value\s*=.*?GetFloat\s*\(\s*""([^""]+)""\s*\)",
                RegexOptions.Multiline);
            ...
            foreach (Match match in enumMatches)
            {
                ...
                if (propertyFixInfos.ContainsKey(propName))
                {
                    ...
                    if (keywords.Count > 0)
                    {
                        ...
                        // 如果之前被标记为Toggle，现在更正为KeywordEnum
                        info.isToggle = false;
                        info.isToggleFromGUI = false;
                        
                        Debug.Log($"识别KeywordEnum: {propName} -> [{string.Join(", ", keywords)}]");
                    }
                }
            }
        }
        ...
        private void AnalyzeFixRequirements()
        {
            foreach (var kvp in propertyFixInfos)
            {
                ...
                // 检查Toggle属性是否需要补全
                if (fixToggleAttributes)
                {
                    // 如果是从GUI识别的Toggle，或已知是Toggle类型
                    if (info.isToggleFromGUI || info.isToggle)
                    {
                        // 有关联的关键字，但当前没有正确的Toggle(KEYWORD)属性
                        if (!string.IsNullOrEmpty(info.associatedKeyword))
                        {
                            bool needsFix = false;
                            ...
                            else if (info.currentAttribute == "Toggle")
                            {
                                // 只有[Toggle]，没有指定关键字
                                needsFix = true;
                            }
                            else if (info.currentAttribute.StartsWith("Toggle(") && 
                                     !info.currentAttribute.Contains(info.associatedKeyword))
                            {
                                // Toggle关键字不匹配
                                needsFix = true;
                            }
                            ...
                            if (needsFix)
                            {
                                // 检查关键字是否存在于Shader中
                                if (detectedKeywords.Contains(info.associatedKeyword) || 
                                    info.isToggleFromGUI) // 从GUI识别的优先信任
                                {
                                    if (info.associatedKeyword.EndsWith("_OFF"))
                                    {
                                        info.suggestedAttribute = "ToggleOff";
                                    ...
                                    info.needsAttributeFix = true;
                                    Debug.Log($"需要补全Toggle属性: {info.propertyName} -> [{info.suggestedAttribute}]");
                                }
                            }
                        }
                        else if (info.isToggleFromGUI && string.IsNullOrEmpty(info.currentAttribute))
                        {
                            // 从GUI识别的Toggle，但没有关联关键字，也没有现有属性
                            // 尝试智能匹配关键字
                            string matchedKeyword = TryMatchKeywordForProperty(info.propertyName);
                            if (!string.IsNullOrEmpty(matchedKeyword))
                            {
                                info.associatedKeyword = matchedKeyword;
                                info.suggestedAttribute = $"Toggle({matchedKeyword})";
                                info.needsAttributeFix = true;
                                Debug.Log($"智能匹配Toggle关键字: {info.propertyName} -> [{info.suggestedAttribute}]");
                            }
                            else
                            {
                                // 没有匹配到关键字，检查是否需要补充简单的[Toggle]
                                bool needsSimpleToggle = false;
                        
                                if (string.IsNullOrEmpty(info.currentAttribute))
                                {
                                    // 当前没有任何属性声明
                                    needsSimpleToggle = true;
                                }
                                else if (!info.currentAttribute.StartsWith("Toggle") && 
                                         !info.currentAttribute.StartsWith("ToggleOff"))
                                {
                                    // 当前有属性但不是Toggle相关的
                                    needsSimpleToggle = true;
                                }
                        
                                if (needsSimpleToggle)
                                {
                                    info.suggestedAttribute = "Toggle";
                                    info.needsAttributeFix = true;
                                    Debug.Log($"补充Toggle属性(无关键字): {info.propertyName} -> [Toggle]");
                                }
                            }
                    ...
                    info.keywordEnumKeywords != null && info.keywordEnumKeywords.Length > 0)
                {
                        ...
                        !info.currentAttribute.StartsWith("KeywordEnum"))
                    {
                        ...
                        if (enumValues.Length > 0)
                        {
                            info.suggestedAttribute = $"KeywordEnum({string.Join(", ", enumValues)})";
                            info.needsAttributeFix = true;
                            Debug.Log($"需要补全KeywordEnum: {info.propertyName} -> [{info.suggestedAttribute}]");
                        }
                    }
                }
            }
        ...
        /// 尝试为属性智能匹配关键字
        /// </summary>
        private string TryMatchKeywordForProperty(string propertyName)
        {
            string propCore = propertyName.TrimStart('_').ToUpper();
            ...
            string cleanPropCore = propCore;
            foreach (var suffix in suffixes)
            {
                if (cleanPropCore.EndsWith(suffix))
                {
                    cleanPropCore = cleanPropCore.Substring(0, cleanPropCore.Length - suffix.Length);
                    break;
                }
            }
            ...
            {
                string keywordCore = keyword.TrimStart('_').Replace("_ON", "").Replace("_OFF", "");
                
                if (keywordCore.Equals(cleanPropCore, StringComparison.OrdinalIgnoreCase) ||
                    keywordCore.Equals(propCore, StringComparison.OrdinalIgnoreCase))
                {
        ...
        private string ProcessPropertyLine(string line)
        {
            ...
            if (info.needsDisplayNameFix && !string.IsNullOrEmpty(info.suggestedDisplayName))
            {
                var displayNameMatch = Regex.Match(newLine, @"(\(\s*"")([^""]*?)(""\s*,)");
                if (displayNameMatch.Success)
                {
                    newLine = newLine.Substring(0, displayNameMatch.Groups[2].Index) +
                             info.suggestedDisplayName +
            ...
            if (addTooltipsAsComments && !string.IsNullOrEmpty(info.tooltip))
            {
                if (!newLine.TrimEnd().EndsWith("//") && !newLine.Contains("//"))
                {
                    newLine = newLine.TrimEnd() + $" // {info.tooltip}";
                }
            }
        ...
        private void ApplyFixes()
        {
            ...
            int displayNameFixed = propertyFixInfos.Values.Count(p => p.needsDisplayNameFix);
            int attributeFixed = propertyFixInfos.Values.Count(p => p.needsAttributeFix);
            
            EditorUtility.DisplayDialog("成功", 
                $"已应用修改到:\n{shaderPath}\n\n" +
                $"修复了 {displayNameFixed} 个显示名称\n" +
                $"补全了 {attributeFixed} 个属性声明", 
                "确定");
